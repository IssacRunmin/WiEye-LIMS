...
//            options.setPassword(passWord.toCharArray());
            ...
            // 设置回调
            client.setCallback(new PushCallback(ClientName,MQTT.LogTOPIC));
            client.connect(options);
            int[] Qos  = {1};
            String[] topic1 = {MQTT.LogTOPIC};
    ...
    public RecvMQTT(final String Topic){
        ClientName = MQTT.GenerateClientName();
        try{
            client = new MqttClient(MQTT.HOST, ClientName, new MemoryPersistence());
            options = new MqttConnectOptions();
...
//            options.setWill(topic, "closeddd".getBytes(), 2, true);
            client.connect(options);
            //订阅消息
            int[] Qos  = {1};
            String[] topic1 = {Topic};
